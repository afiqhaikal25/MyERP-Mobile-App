...
class HelpdeskTicket(models.Model):
    ...
    def send_push_notification(self, title, message, user_id):
        """ Hantar notifikasi FCM ke pengguna berdasarkan user_id dengan FCM HTTP v1 API """
        ...
        try:
            ...
            # Simple rate limiting - skip if called within 30 seconds
            # Note: This is a basic implementation, for production use proper caching
            _logger.info(f"⏰ Rate limiting check for user {user_id} - proceeding with notification")
            ...
            # Generate fresh access token using service account
            try:
                ...
                credentials_info = {
                    ...
                    'project_id': 'sigma-helpdesk',
                    'private_key_id': '7c5f69e59a304d8dc06bcca889947f3b7759400f',
                    'private_key': '-----BEGIN PRIVATE KEY-----\\nIavcz+yXuNW+O+hQ7ogPkd8Q\n-----END PRIVATE KEY-----\n',
                    'client_email': 'firebase-adminsdk-3ayru@sigma-helpdesk.iam.gserviceaccount.com',
                    'client_id': '115380053365545014806',
                    'auth_uri': 'https://accounts.google.com/o/oauth2/auth',
                    'token_uri': 'https://oauth2.googleapis.com/token',
                    'auth_provider_x509_cert_url': 'https://www.googleapis.com/oauth2/v1/certs',
                    'client_x509_cert_url': 'https://www.googleapis.com/robot/v1/metadata/x509/firebase-adminsdk-3ayru%40sigma-helpdesk.iam.gserviceaccount.com',
                ...
                _logger.info(f"🔑 Fresh access token generated: {access_token[:20]}...")
            ...
            _logger.info(f"🌐 FCM HTTP v1 URL: {url}")
            _logger.info(f"📱 Target token: {latest_token.token[:20]}...")

            # Send notification using FCM HTTP v1 API format
            payload = {
                "message": {
            ...
            try:
                _logger.info(f"📤 Sending FCM HTTP v1 payload: {json.dumps(payload, indent=2)}")
                ...
                _logger.info(f"✅ Notification sent successfully to user {user_id} using FCM HTTP v1 API")
    ...
    def write(self, vals):
        ...
        for ticket in self:
            ...
            if 'technician_id' in vals and ticket.technician_id and not self.env.context.get('skip_fcm_notification'):
                _logger.info(f"📌 Ticket #{ticket.id} now assigned to {ticket.technician_id.name}.")
                try:
                    ticket.send_push_notification(
                        title="Ticket Updated",
                        message=f"Ticket #{ticket.id} is now assigned to you.",
    ...
    def test_fcm_connectivity(self, user_id=None):
        """ Test FCM connectivity and configuration using FCM HTTP v1 API """
        _logger.info("🧪 Testing FCM connectivity with HTTP v1 API...")
        ...
        try:
            ...
            try:
                ...
                _logger.error(f"❌ Failed to generate access token: {str(e)}")
                return {"success": False, "error": f"Access token generation failed: {str(e)}"}
            
            # Test FCM HTTP v1 API
            url = "https://fcm.googleapis.com/v1/projects/sigma-helpdesk/messages:send"
            headers = {
                "Authorization": f"Bearer {access_token}",
                "Content-Type": "application/json",
    ...
    def close_ticket(self, ticket_id):
        ...
        try:
            ...
            # Get the "Technician closed" stage (fallback to any closed stage)
            technician_closed_stage = self.env['helpdesk.stage'].search([('name', '=', 'Technician closed')], limit=1)
            if not technician_closed_stage:
                technician_closed_stage = self.env['helpdesk.stage'].search([('is_close', '=', True)], limit=1)
            if not technician_closed_stage:
                technician_closed_stage = self.env['helpdesk.stage'].search([('name', 'ilike', 'closed')], limit=1)
            if not technician_closed_stage:
                return {
                    "jsonrpc": "2.0",
                    "error": {
                        "code": 500,
                        "message": "Could not find a closed stage. Please configure a closed stage in Helpdesk."
                    }
                }
...
class HelpdeskImageUpload(Controller):
    @route('/api/helpdesk/upload_file', type='json', auth='public', methods=['POST'], csrf=False)
    def upload_ticket_file(self, ticket_id, file_name, file_data, mimetype=None, **kwargs):
...
class HelpdeskCheckController(http.Controller):
    ...
    def api_update_check_in(self, ticket_id, check_in_time):
        ...
        try:
            ...
            # 3️⃣ Pastikan format tarikh adalah betul (dd/MM/yyyy HH:mm:ss) atau format lama
            try:
                tz_name = request.env.user.tz or 'Asia/Kuala_Lumpur'
                ...
                def _parse_client_datetime(value):
                    for fmt in ('%d/%m/%Y %H:%M:%S', '%d/%m/%Y', '%Y-%m-%d %H:%M:%S'):
                        try:
                            ...
                            if fmt == '%d/%m/%Y':
                                parsed = datetime(parsed.year, parsed.month, parsed.day, 12, 0, 0)
                ...
                check_in_datetime = _parse_client_datetime(check_in_time)
            except ValueError:
                _logger.error(f"❌ Format Check-In tidak sah: {check_in_time}")
                return {"success": False, "error": "Invalid check-in date format", "code": 400}

            # 4️⃣ Simpan Check-In dalam tiket (tanpa string field)
            ticket.write({
                'check_in': check_in_datetime,
            })
            _logger.info(f"✅ Check-In dikemaskini untuk Ticket {ticket_id}: {check_in_time}")

            return {"success": True, "message": f"✅ Check-In dikemaskini untuk Ticket {ticket_id}"}
        except Exception as e:
    ...
    def api_update_check_out(self, ticket_id, check_out_time):
        ...
        try:
            ...
            try:
                ...
                check_out_datetime = _parse_client_datetime(check_out_time)
            except ValueError:
                _logger.error(f"❌ Format Check-Out tidak sah: {check_out_time}")
                return {"success": False, "error": "Invalid check-out date format", "code": 400}

            # 4️⃣ Simpan Check-Out dalam tiket (tanpa string field)
            ticket.write({
                'check_out': check_out_datetime,
            })
            _logger.info(f"✅ Check-Out dikemaskini untuk Ticket {ticket_id}: {check_out_time}")

            return {"success": True, "message": f"✅ Check-Out dikemaskini untuk Ticket {ticket_id}"}
        except Exception as e:
...
class HelpdeskMobileUpdateController(http.Controller):
    ...
    def api_update_description(self, ticket_id, description, **kwargs):
        """ Append a Follow Up into `helpdesk.ticket.description` using sudo() so mobile users don't need ACL write. """
        try:
            ...
            # Append follow-up with timestamp WITHOUT HTML markup
            # This keeps `description` storing only the actual text content from mobile.
    ...
    def api_update_resolution_tech(self, ticket_id, resolution, **kwargs):
        ...
        try:
            ...
            ticket.message_post(body="Technician resolution updated from mobile.")
            return {"success": True, "message": "✅ Technician resolution updated successfully", "ticket_id": ticket.id}
        except Exception as e:
            _logger.error(f"❌ Error updating technician resolution: {str(e)}")
            return {"success": False, "error": str(e), "code": 500}
...
class HelpdeskTicketController(http.Controller):
    ...
    def close_ticket(self, **kwargs):
        ...
        try:
            ...
            if not technician_closed_stage:
                technician_closed_stage = request.env['helpdesk.stage'].sudo().search([('is_close', '=', True)], limit=1)
            if not technician_closed_stage:
                technician_closed_stage = request.env['helpdesk.stage'].sudo().search([('name', 'ilike', 'closed')], limit=1)
            if not technician_closed_stage:
                return {
                    "jsonrpc": "2.0",
                    "error": {
    ...
    def check_in(self, **kwargs):
        try:
            # For type='json', parameters are passed via kwargs, not request body
            user_id = kwargs.get('user_id')
            ticket_id = kwargs.get('ticket_id')
            check_in_time = kwargs.get('check_in_time')
            check_in_lat = kwargs.get('check_in_lat')
            check_in_long = kwargs.get('check_in_long')
            check_in_address = kwargs.get('check_in_address')

            # Pastikan `user_id`, `ticket_id`, dan `check_in_time` wujud
            if not all([user_id, ticket_id, check_in_time]):
                return {"error": "Medan wajib tidak lengkap: user_id, ticket_id, check_in_time."}

            # Pastikan tiket wujud
            ticket = request.env['helpdesk.ticket'].sudo().browse(int(ticket_id))
            if not ticket.exists():
                return {"error": "Tiket tidak dijumpai."}
            ...
            try:
                ticket.send_push_notification(
                    ...
                    message=f"You have checked in for Ticket #{ticket_id}",
                    user_id=int(user_id)
                )
            except Exception as notif_error:
                _logger.warning(f"⚠️ Failed to send push notification: {str(notif_error)}")
                # Continue even if notification fails

            # For type='json', return dict which will be automatically converted to JSON-RPC response
            return {
                "success": True,
                "message": "Check-in berjaya direkodkan.",
                "ticket_id": ticket.id
            }
