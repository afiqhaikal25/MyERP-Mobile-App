...
class HelpdeskTicket(models.Model):
    ...
    def create(self, vals):
        ...
        if ticket.technician_id:
            _logger.info(f"📌 New ticket '{ticket.name}' assigned to {ticket.technician_id.name}.")
    ...
    def write(self, vals):
        ...
        for ticket in self:
            ...
            # ✅ Hantar notifikasi jika teknisyen berubah
            if 'technician_id' in vals and ticket.technician_id:
            ...
            if ticket.user_id.fcm_token_id:
                ...
                for token in fcm_tokens:
                    ...
                    creds = ServiceAccountCredentials.from_json_keyfile_name(
                        '/mnt/extra-addons/helpdesk/firebase-fcm.json',
...
class HelpdeskTicketController(http.Controller):
    ...
    def fetch_tickets(self, **kwargs):
        try:
            _logger.info(f"🔹 FETCH TICKETS DEBUG: kwargs={kwargs}")
            _logger.info(f"🔹 FETCH TICKETS DEBUG: request.httprequest.data={request.httprequest.data}")
            _logger.info(f"🔹 FETCH TICKETS DEBUG: request.httprequest.args={request.httprequest.args}")
            
            # Extract user_id from URL parameters or JSON body
            user_id = kwargs.get('user_id')
            _logger.info(f"🔹 FETCH TICKETS DEBUG: user_id from kwargs={user_id}")
            
            # If not in URL parameters, try to get from JSON body
            if not user_id and request.httprequest.data:
                try:
                    data = json.loads(request.httprequest.data)
                    user_id = data.get('user_id')
                    _logger.info(f"🔹 FETCH TICKETS DEBUG: user_id from JSON={user_id}")
                except (json.JSONDecodeError, ValueError) as e:
                    _logger.info(f"🔹 FETCH TICKETS DEBUG: JSON decode error={e}")
                    pass
            
            _logger.info(f"🔹 FETCH TICKETS DEBUG: final user_id={user_id}")
...
# --- FCM Token Storage Controller (Unified, Clean) ---
class FcmTokenController(http.Controller):
    @http.route('/store_fcm_token', type='json', auth='user', methods=['POST'])
    def store_fcm_token(self, token=None, **kwargs):
        """
        Store FCM Token from Flutter or other clients (compatible with ticket33.py)
        """
        user = request.env.user
        if not token:
            return {"success": False, "error": "Token is required"}
        request.env['fcm.token'].sudo().store_fcm_token(user.id, token)
        _logger.info(f"✅ Stored FCM token for user {user.id}: {token}")
        return {"success": True, "message": "✅ FCM Token stored successfully"}

    @http.route('/fcm/token', type='json', auth='user', methods=['POST'])
    def create_fcm_token(self, token=None, **kwargs):
        """
        Store FCM Token (new endpoint, preferred for new clients)
        """
        user = request.env.user
        if not token:
            return {"success": False, "error": "Token is required"}
...
class UserImageController(http.Controller):
    ...
    def update_user_image(self, **kwargs):
        try:
            ...
            data = {}
        user_id = data.get('user_id') or kwargs.get('user_id')
        image_base64 = data.get('image_base64') or kwargs.get('image_base64')
        if not user_id or not image_base64:
            return {'success': False, 'error': 'user_id and image_base64 are required'}
        user = request.env['res.users'].sudo().browse(int(user_id))
        if not user.exists():
            return {'success': False, 'error': 'User not found'}
        try:
